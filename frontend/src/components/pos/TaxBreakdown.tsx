  ...
  return (
    <div>
      ...
      >
        {hasBreakdown && (
          expanded
            ? <ChevronDown size={12} className={theme === 'light' ? 'text-muted-foreground/70' : 'text-slate-400'} />
            : <ChevronRight size={12} className={theme === 'light' ? 'text-muted-foreground/70' : 'text-slate-400'} />
        )}
      ...
      {expanded && hasBreakdown && (
        <div className="ml-4 mt-1 space-y-0.5">
          {breakdownArray.map((line, i) => {
            ...
            return (
              <div key={`${title}_${rate}_${i}`} className={`flex justify-between text-xs ${theme === 'light' ? 'text-muted-foreground/70' : 'text-slate-400'}`}>
