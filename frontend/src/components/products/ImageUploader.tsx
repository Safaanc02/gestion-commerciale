      ...
      <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
        <div className="bg-card rounded-2xl max-w-lg w-full overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b">
            ...
            <button type="button" onClick={() => { setMode('idle'); setCropSrc(null); }} className="text-muted-foreground/70 hover:text-muted-foreground">
              <X size={20} />
            </button>
          </div>
  ...
  if (mode === 'url-input') {
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground/70">Only HTTPS URLs supported. Image will be fetched, cropped, and stored locally.</p>
      </div>
    );
  }
      ...
      <div className="space-y-3">
        ...
        <div
          ...
          className={`w-full flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
            isDragActive ? 'border-brand bg-brand/5 text-brand' : 'border-input text-muted-foreground hover:border-brand hover:bg-muted'
          }`}
        >
          <input {...getInputProps()} />
          <Upload size={24} className="mb-2 text-muted-foreground/70" />
        ...
        <div className="flex items-center gap-2 justify-center">
          ...
          <span className="text-xs text-muted-foreground/70 font-medium uppercase px-2">OR USE</span>
          <div className="flex-1 h-px bg-border"></div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          ...
          <button type="button"
            ...
            className="flex items-center gap-2 px-4 py-2 border border-border rounded-lg text-sm text-foreground hover:bg-muted hover:border-input transition-colors"
          >
            <Camera size={16} />
            Camera
          </button>
