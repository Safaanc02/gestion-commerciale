  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          // Logical insets, not physical ones.
          //
          // The spacer above sits in normal flow, so under dir="rtl" it moves
          // to the right — while `left-0` pinned this fixed panel to the
          // physical left. The two landed on opposite sides, the reserved gap
          // was empty and the page content ran underneath the sidebar: in the
          // Arabic interface the cart panel was sliced off at the screen edge.
          // `start`/`end` follow the writing direction, so the panel and its
          // gap always agree.
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 rtl:translate-x-1/2 transition-all ease-linear group-data-[side=left]:-end-4 group-data-[side=right]:start-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
