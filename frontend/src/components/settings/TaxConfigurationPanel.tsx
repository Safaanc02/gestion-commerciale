  ...
  async function saveManualConfig(override = false) {
    ...
    try {
      ...
      if (remapped.length > 0) {
        ...
        toast(`${summary} lost their previous tax category and were reassigned to the new default.`, { icon: <AlertTriangle size={18} className="text-amber-500" /> });
      }
    ...
    <div className="pb-6 max-w-5xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-center justify-between gap-2">
          ...
          {!taxesEnabled && (
            <button type="button" onClick={() => setManualBuilderOpen(false)} className="text-sm text-muted-foreground/70 hover:text-muted-foreground">Hide</button>
          )}
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
            ...
            <div key={category.tempId} className="rounded-lg border border-border bg-muted p-3">
              <div className="flex items-center gap-2">
                ...
                {isOwner && manualCategories.length > 1 && (
                  <button type="button" onClick={() => removeManualCategory(category.tempId)} className="p-2 text-muted-foreground/70 hover:text-red-600" title="Remove category">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
                ...
                {category.components.map((component) => (
                  <div key={component.key} className="flex items-center gap-2 pl-4">
                    ...
                    <input
                      ...
                      disabled={!isOwner}
                      className="w-24 rounded-md border border-border bg-card px-2 py-1.5 text-sm text-right disabled:bg-muted"
                    />
                    {isOwner && category.components.length > 1 && (
                      <button type="button" onClick={() => removeManualComponent(category.tempId, component.key)} className="p-1.5 text-muted-foreground/70 hover:text-red-600" title="Remove component">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </div>
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-border bg-card p-5 text-left"
      >
        <div>
                ...
                {expandedChecklist && (
                  <ol className="mt-3 grid gap-1 rounded-lg border border-border p-3 text-xs sm:grid-cols-2">
                    {detail.active_version.validation.checks.map((check) => (
                      <li key={check.id} className={`flex items-start gap-1.5 ${check.passed ? 'text-muted-foreground' : 'text-red-700'}`}>
                        {check.passed
                ...
                {detail.versions.map((version) => {
                  ...
                  return (
                    <div key={version.id} className="flex items-center justify-between rounded-lg border border-border px-3 py-2 text-sm">
                      <span>
                        v{version.version}
                        <span className="ml-2 text-xs text-muted-foreground/70">{version.status}</span>
                      </span>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        {isOwner && (
          <div className="mt-4 grid gap-3 rounded-lg border border-border bg-muted p-4 sm:grid-cols-3">
            ...
            {needsEntity ? (
              ...
              <div className="rounded-md border border-border bg-muted px-3 py-2 text-sm text-muted-foreground">Store-wide charge</div>
            )}
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[620px] text-left text-sm">
            <thead className="border-b border-border text-xs uppercase text-muted-foreground/70">
            ...
            <tbody>
              {detail?.overrides.map((override) => (
                <tr key={override.id} className="border-b border-border">
                  <td className="py-3 pr-3"><span className="text-xs text-muted-foreground/70">{ENTITY_LABELS[override.entity_type]}</span><br />{override.entity_name || 'Store-wide'}</td>
              ...
              {!detail?.overrides.length && <tr><td colSpan={4} className="py-8 text-center text-muted-foreground/70">No merchant overrides. Official pack behavior is in use.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[680px] text-left text-sm">
            ...
            <tbody>
              {detail?.categories.map((category) => (
                <tr key={category.category_id} className="border-b border-border">
                  <td className="py-3 pr-3"><span className="font-medium">{category.label}</span><br /><code className="text-xs text-muted-foreground/70">{category.category_id}</code></td>
            ...
            <tbody>
              {detail?.rules.map((rule) => (
                <tr key={rule.rule_id} className="border-b border-border">
                  <td className="py-3 pr-3"><span className="font-medium">{rule.label}</span><br /><code className="text-xs text-muted-foreground/70">{rule.rule_id}</code></td>
        ...
        <div className="mt-4 space-y-2">
          {audit.map((row) => (
            <div key={row.id} className="flex items-start gap-3 rounded-lg border border-border px-3 py-3">
              <Clock3 size={15} className="mt-0.5 shrink-0 text-muted-foreground/70" />
          ...
          {!audit.length && <p className="py-6 text-center text-sm text-muted-foreground/70">No tax configuration changes recorded.</p>}
        </div>
      </section>
        </>
      )}
