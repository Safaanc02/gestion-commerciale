...
const orderStatusBadge: Record<string, { bg: string; text: string; labelKey: string }> = {
  ...
  completed: { bg: 'bg-muted', text: 'text-muted-foreground', labelKey: 'orders.completed' },
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand bg-card"
          />
        </div>
        ...
        <select
          ...
          className="px-3 py-2 border border-border rounded-lg text-sm bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
        >
                    ...
                    {linkCustomerOrderId === order.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-1.5 text-sm border border-input rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                          autoFocus
                        />
                        <button
                          onClick={() => {
                      ...
                      <div className="mt-2 space-y-1">
                        {linkCustomerResults.map((customer) => (
                          <button
                            ...
                            className="w-full flex items-center justify-between px-3 py-2 bg-card rounded-lg border border-border hover:border-blue-300 hover:bg-blue-50 transition-colors text-left disabled:opacity-50"
                          >
                            <div>
                            ...
                            {linkingCustomer && <span className="text-xs text-muted-foreground/70">{t('orders.linking')}</span>}
                          </button>
                        ))}
                      </div>
                    )}
                          ...
                          {item.addons && item.addons.length > 0 && (
                            <div className="pl-4 mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
                              {item.addons.map((addon, idx) => (
                                <span key={addon.id ?? `${item.id}-${idx}`} className="text-xs text-muted-foreground/70">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                />
              </div>
...
placeholder={t('orders.managerPin')}
      ...
      {/* Discount Modal */}
      {/* Gated on the setting rather than on however this modal happens to be
          opened — the server refuses the endpoint too, so this is only about
          not showing a control that would fail. */}
      {discountEnabled && discountModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card rounded-xl shadow-xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                />
              </div>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-purple-600">
                    ...
                    {discountModal.type === 'percentage' && discountModal.value > 0 && (
                      <span className="text-muted-foreground/70 ml-1">{t('orders.percentOnSubtotal', { value: discountModal.value })}</span>
                    )}
                  </span>
                  <span className="text-purple-600">
                    -{fmt(
          ...
          <div className="bg-card rounded-xl shadow-xl p-6 w-full max-w-lg mx-4 max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto border border-border rounded-lg mb-3 max-h-48">
              {products
                ...
                .map((product: Product) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 hover:bg-green-50 text-left border-b border-border last:border-0 transition-colors"
                  >
                    <div>
              ...
              {products.filter(p => !productSearch || p.name.toLowerCase().includes(productSearch.toLowerCase())).length === 0 && (
                <div className="px-3 py-4 text-sm text-muted-foreground/70 text-center">{t('orders.noItemsFound')}</div>
              )}
            </div>
                  ...
                  <div key={item.product_id} className="flex items-center gap-2 bg-muted rounded-lg p-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <input
                        ...
                        className="w-full text-xs text-muted-foreground bg-transparent border-0 p-0 focus:outline-none placeholder:text-muted-foreground/50"
                      />
                    </div>
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => handleUpdateSelectionQty(item.product_id, item.quantity - 1)}
                        className="w-6 h-6 rounded bg-border text-muted-foreground text-xs hover:bg-border"
                      >-</button>
