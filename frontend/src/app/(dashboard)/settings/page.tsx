...
import { Settings, Building2, CreditCard, Monitor, Users, Gift, Printer, Share2, FileText, Lock, Smartphone, RefreshCw, Copy, Check, Wifi, Usb, Trash2, Plus, Star, TestTube2, ChefHat, QrCode, CheckCircle2, Database, Cloud, CloudOff, Zap, Percent, KeyRound, AlertTriangle, Wrench, HardDrive, UploadCloud, Hash, X } from 'lucide-react';
...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${value ? 'bg-brand' : 'bg-border'}`}
    >
      <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-card rounded-full shadow transition-transform ${value ? 'translate-x-5' : 'translate-x-0'}`} />
    </button>
  );
}
    ...
    <div>
      <Tabs orientation="vertical" value={activeTab} onValueChange={setActiveTab} className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex items-center gap-3 mb-6">
           ...
           <nav className="flex md:flex-col gap-0.5 overflow-x-auto md:overflow-x-visible border-b md:border-b-0 md:border-r border-border pb-2 md:pb-0 md:pr-2">
            ...
            <div className="hidden md:block px-3 pt-3 pb-2 mt-2 mb-1 border-b border-border">
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">{t('settings.navGroupGeneral')}</p>
            </div>
            ...
            <div className="hidden md:block px-3 pt-4 pb-2 mt-3 mb-1 border-b border-border">
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">{t('settings.navGroupOperations')}</p>
            </div>
            ...
            {/* Customers group */}
            <div className="hidden md:block px-3 pt-4 pb-2 mt-3 mb-1 border-b border-border">
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">{t('settings.navGroupCustomers')}</p>
            </div>
            ...
            <div className="hidden md:block px-3 pt-4 pb-2 mt-3 mb-1 border-b border-border">
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">{t('settings.navGroupData')}</p>
            </div>
            ...
            {/* Account group */}
            <div className="hidden md:block px-3 pt-4 pb-2 mt-3 mb-1 border-b border-border">
              <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">{t('settings.navGroupAccount')}</p>
            </div>
          ...
          <div className="pb-6 max-w-3xl space-y-6">
            ...
            <div className="lg:col-span-2 bg-card rounded-xl border border-border p-6">
                        ...
                        {taxIdFormat && (
                          <p className={`text-xs mt-1 ${
                            form.taxRegistrationNumber && !new RegExp(taxIdFormat.pattern, 'i').test(form.taxRegistrationNumber.trim())
                              ? 'text-red-600' : 'text-muted-foreground/70'
                          }`}>
              ...
              <div className="mt-5 pt-5 border-t border-border">
                <p className="text-sm font-semibold text-foreground mb-3">{t('settings.showOnInvoice')}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {([
                          ...
                          <div key={idx} className="flex flex-col items-center p-4 bg-muted border border-border rounded-lg">
                            ...
                            {ipInfo.qr_data ? (
                              ...
                              <div className="w-40 h-40 bg-muted rounded-lg flex items-center justify-center mb-3">
                                <QrCode size={40} className="text-muted-foreground/70" />
                              </div>
                            )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 items-start">
                      <div className="shrink-0">
                        {posInfo.qr_data_url ? (
                          ...
                          <div className="w-48 h-48 rounded-xl border border-border flex items-center justify-center text-muted-foreground/70">
                            <QrCode size={48} />
                          </div>
                        )}
                      </div>
        ...
        <TabsContent value="kds">
          <div className="pb-6 max-w-3xl space-y-6">
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              {stations.length === 0 ? (
                <p className="text-sm text-muted-foreground/70 py-4 text-center">{t('settings.noStationsYet')}</p>
              ) : (
                      ...
                      <div key={station.id} className="flex items-center justify-between p-3 border border-border rounded-lg">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          <button onClick={() => deleteStation(station.id)}
                            className="p-1.5 text-muted-foreground/70 hover:text-red-600 hover:bg-red-50 rounded">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4 py-2">
                      ...
                      <div>
                        ...
                        {stationCategories.length === 0 ? (
                          <p className="text-xs text-muted-foreground/70">{t('settings.noCategoriesYet')}</p>
                        ) : (
                      ...
                      <div>
                        ...
                        {stationStaff.length === 0 ? (
                          <p className="text-xs text-muted-foreground/70">{t('settings.noStaffYet')}</p>
                        ) : (
                          <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                            {stationStaff.map((u) => (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      loyaltyEnabled ? 'bg-brand' : 'bg-border'
                    }`}
                  >
                  ...
                  <div className="pt-4 border-t border-border flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="0"
                        className="w-20 px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-brand focus:border-brand transition-shadow text-right"
                      />
                ...
                {loyaltyEnabled && globalRateCandidates > 0 && (
                  <div className="pt-4 border-t border-border">
                    ...
                    <button
                      ...
                      className="mt-3 px-4 py-2 text-sm font-medium rounded-lg border border-input hover:bg-muted disabled:opacity-50"
                    >
                      {applyingGlobalRate
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      discountRequiresApproval ? 'bg-brand' : 'bg-border'
                    }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-card transition-transform ${
        ...
        <TabsContent value="account">
          <div className="pb-6 max-w-3xl space-y-6">
            ...
            {currentTenant?.role === 'owner' && (
              <div className={`rounded-xl border p-6 ${cloudAccount?.email && !cloudAccount.verified ? 'border-red-200 bg-red-50/40' : 'border-border bg-card'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="font-semibold text-foreground">Contact email</h2>
                    <p className="mt-1 text-sm text-muted-foreground">{cloudAccount?.email || user?.email || 'No cloud contact email'}</p>
                  </div>
                ...
                <p className="mt-3 text-sm text-muted-foreground">Verification is important for product service notices, security updates, and other account communication.</p>
            ...
            {currentTenant?.role === 'owner' && (
              <div className="rounded-xl border border-border bg-card p-6">
                ...
                {cloudAccount?.deletion_request && (
                  <div className={`mt-4 rounded-lg border p-3 text-sm ${cloudAccount.deletion_request.status === 'pending' ? 'border-amber-200 bg-amber-50 text-amber-900' : cloudAccount.deletion_request.status === 'approved' ? 'border-green-200 bg-green-50 text-green-800' : 'border-border bg-muted text-foreground'}`}>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Button variant="outline" onClick={async () => {
                    if (!await confirm('Stop all Tijarti cloud services, identified diagnostics, and future anonymous telemetry on this device? Local POS data will remain available.')) return;
        ...
        <TabsContent value="receipts-printers">
          <div className="pb-6 max-w-3xl space-y-6">
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              {!showPrinterForm && (
                <div className="mb-5">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">{t('settings.installedOnThisComputer')}</h3>
                  {detectingPrinters && detectedPrinters.length === 0 ? (
                    <div className="py-6 text-center text-muted-foreground/70 text-sm">{t('settings.scanningForPrinters')}</div>
                  ) : detectedPrinters.length === 0 ? (
                    <div className="py-6 text-center text-muted-foreground/70 text-sm border border-dashed border-border rounded-lg">
                    ...
                    <div className="space-y-2">
                      {detectedPrinters.map((p) => {
                        ...
                        const dotColor = p.status === 'idle' ? 'bg-green-500' : p.status === 'printing' ? 'bg-yellow-500' : 'bg-border';
                        ...
                        return (
                          <div key={p.name} className="flex items-center gap-3 rounded-xl border border-border p-3">
                            ...
                            {alreadyAdded ? (
                              <span className="text-xs text-muted-foreground/70 px-3 py-1.5 flex items-center gap-1">
                  ...
                  <div key={p.id} className={`flex items-center gap-3 rounded-xl border p-4 ${p.is_default ? 'border-brand bg-brand/5' : 'border-border'}`}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => testPrinterHw(p)} disabled={testingPrinterId === p.id}
                        ...
                        className="p-2 rounded-lg hover:bg-muted text-muted-foreground/70 hover:text-foreground disabled:opacity-40">
                        <TestTube2 size={15} />
                      </button>
                      {p.is_default !== 1 && (
                        <button onClick={() => setDefaultPrinter(p.id)} title={t('settings.setAsDefault')}
                          className="p-2 rounded-lg hover:bg-yellow-50 text-muted-foreground/70 hover:text-yellow-600">
                          <Star size={15} />
                        </button>
                      )}
                      <button onClick={() => openEditPrinter(p)} title={t('settings.edit')}
                        className="p-2 rounded-lg hover:bg-muted text-muted-foreground/70 hover:text-foreground">
                        <Settings size={15} />
                      </button>
                      <button onClick={() => deletePrinterHw(p.id)} title={t('settings.delete')}
                        className="p-2 rounded-lg hover:bg-red-50 text-muted-foreground/70 hover:text-red-600">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </div>
                ...
                <div className="mt-5 pt-5 border-t border-border">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {printerForm.connection_type === 'usb' && (
                      <div className="md:col-span-2">
                        <label className="block text-xs text-muted-foreground mb-1">{t('settings.usbDevicePath')}</label>
                        <input type="text" value={printerForm.usb_device_path}
                          ...
                          className="w-full px-3 py-2 text-sm border border-border rounded-lg outline-none focus:ring-2 focus:ring-brand" />
                        <p className="text-xs text-muted-foreground/70 mt-1">{t('settings.usbDevicePathHint')}</p>
                      </div>
                    )}
            ...
            <div className="pt-4 border-t border-border">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground/70">{t('settings.tabPrinting')}</h2>
            </div>
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="font-medium text-foreground mb-2">{t('settings.webPrintSize')}</p>
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-semibold text-foreground">{t('settings.whatsappSharing')}</h2>
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-foreground">{t('settings.enableWhatsappShare')}</p>
                  <p className="text-sm text-muted-foreground">{t('settings.enableWhatsappShareHint')}</p>
                </div>
                  ...
                  return (
                    <button key={card.id} onClick={() => setBillForm((p) => ({ ...p, billTemplate: card.id }))}
                      className={`text-left rounded-xl border-2 p-4 transition-all ${
                        isSelected ? 'border-brand bg-brand/5' : 'border-border hover:border-input bg-card'
                      }`}>
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              <h2 className="font-semibold text-foreground mb-4">{t('settings.footerMessage')}</h2>
              <div>
                ...
                <textarea id="footer-message" rows={2}
                  ...
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg outline-none focus:ring-2 focus:ring-brand resize-none" />
                <p className="text-xs text-muted-foreground/70 mt-1">{t('settings.footerMessageHint')}</p>
              </div>
            </div>
          </div>
          </div>
          ...
          <div className="pb-6 max-w-3xl space-y-6">
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-sm bg-muted text-foreground rounded-lg hover:bg-border font-medium"
                >
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="p-1.5 text-muted-foreground/70 hover:text-muted-foreground rounded-lg hover:bg-muted disabled:opacity-50"
                    ...
                    <div key={backup.path} className="flex items-center justify-between py-3 gap-3">
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 text-xs bg-muted text-foreground rounded-lg hover:bg-border font-medium"
                        >
                          {t('settings.restoreBackup')}
                        </button>
                        <button
                          onClick={() => handleDeleteBackup(backup)}
                          className="p-1.5 text-muted-foreground/70 hover:text-red-600 rounded-lg hover:bg-red-50"
              ...
              {!googleDriveStatus.configured ? (
                <div className="bg-muted rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-2">
                  <div className="p-3 bg-card rounded-full shadow-sm">
                    <HardDrive className="w-6 h-6 text-muted-foreground/70" />
                  </div>
                  ...
                  <div className="rounded-lg border border-border px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-2">
                      {googleDriveStatus.connected ? (
                        ...
                        <CloudOff size={16} className="text-muted-foreground/70 shrink-0" />
                      )}
                      <div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:ring-2 focus:ring-brand outline-none disabled:opacity-50"
                          >
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              <div className="flex gap-2">
                <label
                  htmlFor="import-file"
                  className="px-5 py-2 text-sm bg-muted text-foreground rounded-lg hover:bg-border cursor-pointer font-medium"
                >
          ...
          <div className="pb-6 max-w-3xl space-y-6">
            {!whatsappEnabled ? (
              ...
              <div className="bg-card rounded-xl border border-border p-6 flex items-center justify-between gap-4">
                <div>
                  <p className="font-semibold text-foreground">{t('whatsapp.settings.enabled')}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{t('whatsapp.settings.enabledHint')}</p>
                </div>
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    className="mt-0.5 rounded border-input text-brand focus:ring-brand"
                  />
                  <div>
                    <span className="text-sm font-medium text-foreground block">{t('settings.enableBillSync')}</span>
                    <p className="text-xs text-muted-foreground mt-1">{t('settings.enableBillSyncHint')}</p>
                  </div>
                </label>

                    {cloudSettings.cloud_last_sync && (
                      <p className="text-xs text-muted-foreground/70">{t('settings.lastSync', { time: formatDateTime(cloudSettings.cloud_last_sync) })}</p>
                    )}
                  </div>
                </>
              )}
                ...
                <div className="flex flex-col sm:flex-row gap-5 items-start border border-border rounded-xl p-5">
                  <div className="shrink-0">
                    {revflo.qr_data_url ? (
                      ...
                      <div className="w-28 h-28 rounded-lg border border-border flex items-center justify-center text-muted-foreground/70">
                        <QrCode size={32} />
                      </div>
                    )}
                  </div>
                    ...
                    <div className="space-y-2">
                      {pairedDevices.map((d) => (
                        <div key={d.id} className="bg-muted border border-border rounded-lg px-4 py-3 text-sm">
                          ...
                          {d.user_agent && (
                            <p className="text-xs text-muted-foreground/70 mt-1 truncate" title={d.user_agent}>{d.user_agent}</p>
                          )}
                        </div>
                      ))}
                    </div>
        ...
        <TabsContent value="about">
          <div className="pb-6 max-w-3xl space-y-6">
            <div className="bg-card rounded-xl border border-border p-6">
              <h2 className="font-semibold text-foreground mb-4">{t('settings.aboutTijarti')}</h2>
              <p className="text-sm text-muted-foreground mb-6">
                {t('settings.aboutDescription')}
              </p>
              <div className="space-y-3">
                <a href="https://github.com/FreeOpenSourcePOS/Tijarti" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-brand hover:underline">
            ...
            <div className="bg-card rounded-xl border border-border p-6">
              ...
              {!moreAppsLoading && (
                <div className="space-y-4">
                    ...
                    <div key={app.id} className="flex flex-col sm:flex-row gap-5 items-start border border-border rounded-xl p-5">
                      <div className="shrink-0">
                        {app.qr_data_url ? (
                          ...
                          <div className="w-32 h-32 rounded-lg border border-border flex items-center justify-center text-muted-foreground/70">
                            <QrCode size={36} />
                          </div>
                        )}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {!app.available && (
                            <span className="text-xs font-medium text-muted-foreground/70 bg-muted px-2 py-0.5 rounded-full">{t('settings.comingSoon')}</span>
                          )}
                        </div>
                  ...
                  {moreApps.length === 0 && (
                    <p className="text-sm text-muted-foreground/70 text-center py-10">{t('settings.noAppsToShow')}</p>
                  )}
                </div>
              )}
            </div>
            ...
            {updateStatus && updateStatus.status !== 'store' && updateStatus.status !== 'linux-managed' && (
              <div className={`p-4 rounded-lg mb-4 ${
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {updateStatus.status === 'error' && <X size={14} className="text-red-600" />}
                  {updateStatus.status === 'dev-mode' && <AlertTriangle size={14} className="text-yellow-600" />}
